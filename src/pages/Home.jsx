...
const JewelViewer2 = ({diamonds,sideDaimandStonType, metal, shankurl = '3D Website', halourl = '3D Website'}) => {
  ...
  useEffect(() => {
      ...
      viewer
        ...
        .then(async(viewer) => {
          ...
          var materialConfiguratorPlugin = await viewer.getOrAddPlugin(MaterialConfiguratorPlugin);
          ...
          await viewer.getManager().importer.importPath('./assets/3D Websitetest2.vjson', {processImported: true}).then(() => {
          })
          ...
          document.querySelectorAll(".object").forEach((el) => {
            ...
            el.addEventListener("click", async () => {
              ...
              const obj = viewer.scene.findObjectsByName(config.utils.getName(category))[0]?.modelObject
              if(!obj) return
              var value = metal;
              var gem01 = diamonds;
              ...
              // console.log(gem01);
              // setTimeout(() => {
                document.querySelectorAll('.metal-selection.active').forEach(box => {
                  // if (box.textContent == metal) {
                      box.click();
                  // }
                });
                document.querySelectorAll('#GEM01 .box-ring-selection-box.active').forEach(box => {
                  // if (box.textContent == gem01) {
                      box.click();
                  // }
                });
                document.querySelectorAll('#GEM02 .box-ring-selection-box.active').forEach(box => {
                  // if (box.textContent == gem02) {
                      box.click();
                  // }
                });
                document.querySelectorAll('#GEM03 .box-ring-selection-box.active').forEach(box => {
                  // if (box.textContent == gem03) {
                      box.click();
                  // }
                });
                document.querySelectorAll('#GEM04 .box-ring-selection-box.active').forEach(box => {
            ...
            if(index === 0) el.click()
              ...
              // console.log(gem01);
              setTimeout(() => {
                document.querySelectorAll('.box-ring-selection-box').forEach(box => {
                  if (box.textContent == value) {
                      box.click();
                  }
                });
                document.querySelectorAll('#GEM01 .box-ring-selection-box').forEach(box => {
                  if (box.textContent == gem01) {
                      box.click();
                  }
                });
                document.querySelectorAll('#GEM02 .box-ring-selection-box').forEach(box => {
                  if (box.textContent == gem02) {
                      box.click();
                  }
                });
                document.querySelectorAll('#GEM03 .box-ring-selection-box').forEach(box => {
                  if (box.textContent == gem03) {
                      box.click();
                  }
                });
                document.querySelectorAll('#GEM04 .box-ring-selection-box').forEach(box => {
                  if (box.textContent == gem04) {
                      box.click();
                  }
                });
          ...
          document.getElementById('front-camera-button').addEventListener('click', () => {
            const cameraX = document.getElementById('viewer-3d').getAttribute('cameraX');
            const cameraY = document.getElementById('viewer-3d').getAttribute('cameraY');
            const cameraZ = document.getElementById('viewer-3d').getAttribute('cameraZ');
            viewer.scene.activeCamera.position.set(cameraX/2, cameraY+2, cameraZ*10);
          });
          document.getElementById('left-camera-button').addEventListener('click', () => {
            const cameraX = document.getElementById('viewer-3d').getAttribute('cameraX');
            const cameraY = document.getElementById('viewer-3d').getAttribute('cameraY');
            const cameraZ = document.getElementById('viewer-3d').getAttribute('cameraZ');
            viewer.scene.activeCamera.position.set(cameraX * 10, cameraY + 2, cameraZ / 2);
          });
          var value = metal;
          var gem01 = diamonds;
          var gem02 = sideDaimandStonType[0];
  ...
  return (
    <div id="ring-3d-output" className="sticky" style={{ width: "100%" }}>
      <canvas
        ...
        style={{ width: "100%", height: "calc(100vw * 0.3)", zIndex: 1, display: "block" }}
      ...
      ></canvas>
      <button id="front-camera-button" style={{ opacity: 0 }}>Front Camera</button>
      <button id="left-camera-button" style={{ opacity: 0 }}>Left Camera</button>
    </div>
  );
};
...
function Home() {
  ...
  const [sideDaimandStonType, setsideDaimandStonType] = useState(['Diamond', 'Diamond', 'Diamond', 'Diamond', 'Diamond', 'Diamond']);
  ...
  const [retailers, setRetailers] = useState(DEFAULT_RETAILERS.jewelry_retailers);
  ...
  useEffect(() => {
    const fetchRetailers = async () => {
      ...
      try {
        const response = await fetch('https://amgdynamics.horizonbeam.com/retailer-list/123');
        if (!response.ok) {
    ...
    const checkToken = async () => {
      ...
      if (token) {
        if(token && token.length > 20){
          try{
            const response = await fetch(`https://amgdynamics.horizonbeam.com/get-user-all-data-iframe/${token}`);
          ...
          try{
            const response = await fetch(`https://amgdynamics.horizonbeam.com/check-amgdynamics/${token}`);
            ...
            if (data.user ){
              ...
              try {
                ...
                const response = await fetch(`https://amgdynamics.horizonbeam.com/get-user-all-data/${userId}`);
                ...
                if (data.user) {
                  ...
                  localStorage.setItem('email_send_from_brevo', data.user.email_send_from_brevo);
                  setRetailerbrevo_id(data.user.brevo_id);
                  setRetailerbrevo_key(data.user.brevo_key);
                  setCheckUserRegister(true);
                } else {
  ...
  const handlesetDaimandStonType = (value) => {
    ...
    const metalDiv = Array.from(divs).find(div => div.textContent.includes(`Gem 01`));
  ...
  const handlesetsideDaimandStonType = (value, index) => {  
    ...
    const metalDiv = Array.from(divs).find(div => div.textContent.includes(`Gem 0${gem}`));
  ...
  const handlesetRingmetal = (value) => {
    setRingmetal(value);
    const divs = document.querySelectorAll(".customContextGridHeading");
    const metalDiv = Array.from(divs).find(div => div.textContent.includes("METAL01"));
    const metalDiv2 = Array.from(divs).find(div => div.textContent.includes("METAL02"));
    ...
    if (metalDiv) {
      ...
      if ( value === 'Platinum' || value === '14K White Gold' || value === '18K White Gold' ) {
        if (firstSibling) {
          firstSibling.click();
        }
      }
  ...
  if(Daimandsetting === '4 PRONG'){
    filteredDiamondtypeitems = Diamondtypeitems.filter(item => item.name === 'Pear' || item.name === 'Princess' || item.name === 'Radiant');
    if(Daimandtype != 'Pear.glb' && Daimandtype != 'Princess.glb' && Daimandtype != 'Radiant.glb'){
      setDaimandtype('Pear.glb');
    }
  }else if(Daimandsetting === '6 PRONG'){
    filteredDiamondtypeitems = Diamondtypeitems.filter(item => item.name === 'Marquise' || item.name === 'Oval' || item.name === 'Pear' || item.name === 'Round');
    if(Daimandtype != 'Marquise.glb' && Daimandtype != 'Oval.glb' && Daimandtype != 'Pear.glb' && Daimandtype != 'Round.glb'){
      setDaimandtype('Marquise.glb');
    }
  }else if(Daimandsetting === 'BEZEL'){
    filteredDiamondtypeitems = Diamondtypeitems.filter(item => item.name === 'Cushion Square' || item.name === 'Cushion' || item.name === 'Emerald' || item.name === 'Marquise' || item.name === 'Oval' || item.name === 'Pear' || item.name === 'Princess' || item.name === 'Radiant' || item.name === 'Round');
    if(Daimandtype != 'Cushion Square.glb' && Daimandtype != 'Cushion.glb' && Daimandtype != 'Emerald.glb' && Daimandtype != 'Marquise.glb' && Daimandtype != 'Oval.glb' && Daimandtype != 'Pear.glb' && Daimandtype != 'Princess.glb' && Daimandtype != 'Radiant.glb' && Daimandtype != 'Round.glb'){
    ...
    filteredDiamondtypeitems = Diamondtypeitems.filter(item => item.name === 'Cushion' || item.name === 'Emerald' || item.name === 'Marquise' || item.name === 'Oval' || item.name === 'Pear' || item.name === 'Princess' || item.name === 'Radiant' || item.name === 'Round');
    if(Daimandtype != 'Cushion.glb' && Daimandtype != 'Emerald.glb' && Daimandtype != 'Marquise.glb' && Daimandtype != 'Oval.glb' && Daimandtype != 'Pear.glb' && Daimandtype != 'Princess.glb' && Daimandtype != 'Radiant.glb' && Daimandtype != 'Round.glb'){
  ...
  const handleCreateTransactionSubmit = async () => {
    ...
    formData.append('particular', JSON.stringify({
      item: "Gold Ring",
      price: pricing,
    ...
    .then(data => {
      ...
      .then(response => {
        if (!response.ok) {
          throw new Error('Failed to send transaction ID to customer by Brevo');
        }
        return response.json();
      })
      .then(data => {
      ...
      .catch(error => {
        console.error('Error sending transaction ID to customer by Brevo:', error);
      });
  ...
  const handleContactSubmit = async () => {
    ...
    try {
      await import('html2canvas').then(html2canvasModule => {
        ...
        setTimeout(() => {
          html2canvasModule.default(document.getElementById('viewer-3d')).then(canvas => {
  ...
  const handlesetPutOnMyWebsite = async () => {
    ...
    .then(() => {
      return fetch(`https://amgdynamics.horizonbeam.com/get-user-all-data/${retailerid}`);
    })
    .then(response => {
      if (!response.ok) {
        // Handle specific error codes
  ...
  useEffect(() => {
    ...
    try {
      const fetchDiamonds = async () => {
          const response = await fetch('https://amgdynamics.horizonbeam.com/diamond-list/product_sku');
          const data = await response.json();
  ...
  return (
    <div className='daimand-body active'>
      {loading && <Loader />}
      <header className="header flex justify-center" style={{ background: 'url(top_header_bg.png) no-repeat', backgroundSize: 'cover', backgroundPosition: 'center',  margin: '0px -15px 0px -15px'}}>
        <div className="logo p-5 text-center">
          <img src="logo2.png" alt="Logo" style={{ height: '120px' }} />
        </div>
      </header>
      <div  style={{ backgroundColor: 'rgb(108, 34, 93)', padding: '13px 15px', margin: '0px -15px 50px -15px' }}>
        <div className="flex justify-end mx-auto" style={{ maxWidth: '1300px' }}>
        {retailersapi == 'true' && ifremcheck && CheckUserRegister && ( 
          <a href="/my-transactions" className="text-white">Account</a>
        )}
        </div>
      </div>
      {/* <div className={`main-step-container
        ...
        <div className={`box
                    ${ activestep == 1 || activestep == 2 || activestep == 3 ? "active" : "" }`} >
          <div className='step-number'>1</div> 
          <div className='step-text'>
            <small>Choose a</small>
            <div className='step-big-text'>Setting</div>  
          </div> 
        </div>
        <div className={`box
                    ${ activestep == 2 || activestep == 3 ? "active" : "" }`}>
      ...
      <div className={`flex flex-wrap main-contant-container active items-start `}>
        <div className='w-full lg:w-2/5 p-3.5 sticky top-0 bg-white' style={{ zIndex: '98', paddingTop: '20px' }}>
          <JewelViewer2 
          ...
          />
          {retailersapi == 'true' && ifremcheck && CheckUserRegister == false && (
        <div style={{ textAlign: 'center', marginTop: '20px' }}>
          <button type="button" className="add-to-website-button" onClick={() => setShowPutOnMyWebsitePopup(true)}>
            Put On My Website
          </button>
        </div>
          )}
        ...
        < div id="ui">
            <div>
              <span>Basket</span>
              <button class="object" data-category="Basket" data-index="0" >Cushion Halo</button>
              <button class="object" data-category="Basket" data-index="1" >Empty</button>
              <button class="object" data-category="Basket" data-index="2" >Pear Halo</button>
            </div>
            <div>
              <span>Ring</span>
              <button class="object" data-category="Ring" data-index="0" >Alliance</button>
              <button class="object" data-category="Ring" data-index="1" >Baguette</button>
            </div>
            <div>
              <span>Metal</span>
              <button class="material" data-category="Metal" data-index="0" >rose</button>
              <button class="material" data-category="Metal" data-index="1" >white</button>
            </div>
            <div>
              <span>Diamond</span>
              <button class="diamond" data-category="Diamond" data-index="0" >white</button>
              <button class="diamond" data-category="Diamond" data-index="1" >sapphire</button>
              <button class="diamond" data-category="Diamond" data-index="2" >emerald</button>
              <button class="diamond" data-category="Diamond" data-index="3" >brown</button>
            </div>
          </div>
        </div>
          ...
          <div className='daimang-configurator-selector-body'>
            <div className='daimang-configurator-body'>
              <div className='flex flex-wrap mb-5'>
                ...
                <div className='w-full md:w-3/4 lg:w-full xl:w-3/4 py-3 bg-gray-100 mb-3 rounded-sm' style={{ maxWidth: '800px' }}>
                  <div className='shank-band-type-container'>
                    <Swiper
                    ...
                    >
                      {Shankbandtypeitems.map((item, index) => (
                        <SwiperSlide key={index}>
                          <div className={`shank-band-type
                      ${ Shankbandtype == item ? "active" : "" }`} onClick={() => handlesetShankbandtype(item)}>{item}</div>
                        </SwiperSlide>
                      ))}
                    </Swiper>
                  </div>
              ...
              <div className='flex flex-wrap mb-5'>
                ...
                <div className='w-full md:w-3/4 lg:w-full xl:w-3/4 pb-5 border-b border-gray-200 pb-3'>
                  <div className='arrow-slider-wrap'>
                    <div id='RingStyleDesignPrev' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronLeft} /></div>
                      <Swiper
                      ...
                      >
                        {RingStyleDesigntypeitems.map((item, index) => (
                          <SwiperSlide key={index}>
                            <div className={`box-ring-selection-box ${ RingStyleDesigntype == item.url ? "active" : "" }`} onClick={() => handlesetRingStyleDesigntype(item.url)} RingStyleDesigntype-item={`${item.url}`}>
                              <div className='box-ring-img-bundal'>
                                  <img src='./assets/img/Traditional_Solitaire.svg' className='main-img'/>
                                  <img src='./assets/img/Traditional_Solitaire_Helper.svg' className='helper-img'/>
                              </div>
                      ...
                      <div id='RingStyleDesignNext' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronRight} /></div>
                    </div>
                  </div>
              </div>
              <div className='flex flex-wrap mb-5 '>
                ...
                <div className='w-full md:w-3/4 lg:w-full xl:w-3/4 pb-5  border-b border-gray-200'>
                  <div className='arrow-slider-wrap'>
                    <div id='sideDaimandCaratPrev' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronLeft} /></div>
                      <Swiper
                        className="padding-swiper"
                        modules={[Navigation]}
                        breakpoints={{
                      ...
                      >
                        {sideDaimandCaratitems.map((item, index) => (
                          <SwiperSlide key={index}>
                            <div className={`box-ring-selection-box
                        ${ sideDaimandCarat == item ? "active" : "" }`} onClick={() => handlesetsideDaimandCarat(item)}>{item}</div>
                          </SwiperSlide>
                        ))}
                      </Swiper>
                    <div id='sideDaimandCaratNext' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronRight} /></div>
                  </div>
                </div>
              </div>
              <div className='flex flex-wrap mb-5'>
                ...
                <div className='w-full md:w-3/4 lg:w-full xl:w-3/4 pb-5  border-b border-gray-200'>
                  <div className='arrow-slider-wrap'>
                    <div id='sideDaimandlengthPrev' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronLeft} /></div>
                      <Swiper
                        className="padding-swiper"
                        modules={[Navigation]}
                        spaceBetween={0}
                      ...
                      >
                        {sideDaimandlengthitems.map((item, index) => (
                          <SwiperSlide key={index}>
                            <div className={`box-ring-selection-box
                        ${ sideDaimandlength == item ? "active" : "" }`} onClick={() => handlesetsideDaimandlength(item)}>{item}</div>
                          </SwiperSlide>
                        ))}
                      </Swiper>
                    <div id='sideDaimandlengthNext' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronRight} /></div>
                  </div>
                </div>
              </div>
              <div className='flex flex-wrap mb-5'>
                ...
                <div className='w-full md:w-3/4 lg:w-full xl:w-3/4 pb-5  border-b border-gray-200'>
                  <div className='arrow-slider-wrap'>
                    <div id='DaimandtypePrev' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronLeft} /></div>
                      <Swiper
                        className="padding-swiper"
                        modules={[Navigation]}
                        slidesPerView={2}
                      ...
                      >
                        {filteredDiamondtypeitems.map((item, index) => (
                          <SwiperSlide key={index}>
                            <div className={`box-ring-selection-box  ${`./assets/all/${Daimandsetting}/${item.url}` ? "test-active" : ""} ${ Daimandtype == item.url ? "active" : "" }`} onClick={() => handlesetDaimandtype(item.url)} Daimandtype-item={`${item.url}`}>
                              <div className='box-ring-img-bundal'>
                    ...
                    <div id='DaimandtypeNext' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronRight} /></div>
                  </div>
                </div>
              </div>
              <div className='flex flex-wrap mb-5'>
                ...
                <div className='w-full md:w-3/4 lg:w-full xl:w-3/4 pb-5  border-b border-gray-200'>
                  <div className='arrow-slider-wrap'>
                    <div id='DaimandCaratPrev' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronLeft} /></div>
                      <Swiper
                        className="padding-swiper"
                        modules={[Navigation]}
                        breakpoints={{
                      ...
                      >
                        {DaimandCaratitems.map((item, index) => (
                          <SwiperSlide key={index}>
                            <div className={`box-ring-selection-box
                        ${ DaimandCarat == item ? "active" : "" }`} onClick={() => handlesetDaimandCarat(item)}>{item}</div>
                          </SwiperSlide>
                        ))}
                      </Swiper>
                    <div id='DaimandCaratNext' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronRight} /></div>
                  </div>
                </div>
              </div>
              <div className='flex flex-wrap mb-5'>
                ...
                <div className='w-full md:w-3/4 lg:w-full xl:w-3/4 pb-5  border-b border-gray-200'>
                  <div className='arrow-slider-wrap'>
                    <div id='DaimandsettingPrev' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronLeft} /></div>
                      <Swiper
                        className="padding-swiper"
                        modules={[Navigation]}
                        breakpoints={{
                      ...
                      >
                        {Daimandsettingitems.map((item, index) => (
                          <SwiperSlide key={index}>
                            <div className={`box-ring-selection-box ${ Daimandsetting == item ? "active" : "" }`} onClick={() => handlesetDaimandsetting(item)} Daimandsetting-item={`${item}`}>
                              <div className='box-ring-img-bundal'>
                                  <img src='./assets/img/Round_Classic_6_Prong.svg' className='main-img'/>
                                  <img src='./assets/img/Round_Classic_6_Prong_Helper.svg' className='helper-img'/>
                              </div>
                              <div className='box-ring-selection-item-text'>
                                {item}
                              </div>
                    ...
                    <div id='DaimandsettingNext' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronRight} /></div>
                  </div>
                </div>
              </div>
              <div className='flex flex-wrap mb-5'>
                ...
                <div className='w-full md:w-3/4 lg:w-full xl:w-3/4 pb-5  border-b border-gray-200'>
                  <div className='arrow-slider-wrap'>
                    <div id='RingmetalsettingPrev' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronLeft} /></div>
                      <Swiper
                        className="padding-swiper"
                        modules={[Navigation]}
                        breakpoints={{
                      ...
                      >
                        {Ringmetalitems.map((item, index) => (
                          <SwiperSlide key={index}>
                            <div className={`metal-selection box-ring-selection-box
                        ${ Ringmetal == item.name ? "active" : "" }`} onClick={() => handlesetRingmetal(item.name)}>
                            <div style={{
                              height: '40px',
                              width: '40px',
                    ...
                    <div id='RingmetalsettingNext' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronRight} /></div>
                  </div>
                </div>
              </div>
              <div className='flex flex-wrap mb-5' id="GEM01">
                <div className='w-full md:w-1/4 lg:w-full xl:w-1/4 content-center pb-5 font-medium'>
                  Center Stone Type
                </div>
                <div className='w-full md:w-3/4 lg:w-full xl:w-3/4 pb-5  border-b border-gray-200'>
                  <div className='arrow-slider-wrap'>
                    <div id='DaimandStonTypePrev' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronLeft} /></div>
                      <Swiper
                        className="padding-swiper"
                        modules={[Navigation]}
                        navigation={{
                      ...
                      >
                        {DaimandStonTypeitems.map((item, index) => (
                          <SwiperSlide key={index}>
                            <div className={` box-ring-selection-box ${ DaimandStonType == item.name ? "active" : "" }`} onClick={() => handlesetDaimandStonType(item.name)}>
                              <div className='box-ring-img-bundal'>
                                  <img src={item.img} className='main-img' style={{ height: '45px', width: '45px',borderRadius: '100px' }} />
                                  <img src='./assets/img/Traditional_Solitaire_Helper.svg' className='helper-img' style={{ height: '45px', width: '45px' }} />
                              </div>
                              <div className='box-ring-selection-item-text'>
                                {item.name}
                              </div>
                      ...
                      <div id='DaimandStonTypeNext' className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronRight} /></div>
                    </div>
                  </div>
              </div>
              {}
              {gemcount.map((testitem, index) => (
              <div className={`flex flex-wrap ${Daimandsetting == 'Classic Prong' ? 'gray-out-active' : ''}`} id={`GEM0${index + 2}`}>
                <div className='w-full md:w-1/4 lg:w-full xl:w-1/4 content-center pb-5 font-medium'>
                  Gem {index + 1}
                </div>
                <div className='w-full md:w-3/4 lg:w-full xl:w-3/4 py-3'>
                  <div className='arrow-slider-wrap'>
                    <div id={`sideDaimandStonTypePrev${index}`} className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronLeft} /></div>
                      <Swiper
                        className="padding-swiper"
                        modules={[Navigation]}
                        navigation={{
                      ...
                      >
                        {sideDaimandStonTypeitems.map((item, index) => (
                          <SwiperSlide key={index}>
                            <div className={`box-ring-selection-box ${ sideDaimandStonType[testitem] == item.name ? "active" : "" }`} onClick={() => handlesetsideDaimandStonType(item.name, testitem)}>
                              <div className='box-ring-img-bundal'>
                                  <img src={item.img} className='main-img' style={{ height: '45px', width: '45px',borderRadius: '100px' }} />
                                  <img src='./assets/img/Traditional_Solitaire_Helper.svg' className='helper-img'/>
                              </div>
                      ...
                      <div id={`sideDaimandStonTypeNext${index}`} className='slider-custom-arrow'><FontAwesomeIcon icon={faChevronRight} /></div>
                    </div>
                  </div>
              </div>
            ))}
                  ...
                  <div className='flex flex-wrap'>
                    <div className='w-full md:w-2/4 content-center'>
                      <div className='matching-band-selection border-0 md:border-r-2'>
                        <span className={`${ matchingBand == 1 ? "active" : "" }`} onClick={() => handleMatchingBand(1)}>1</span>
                        <span className={`${ matchingBand == 2 ? "active" : "" }`} onClick={() => handleMatchingBand(2)}>2</span>
                        <span className={`${ matchingBand == 3 ? "active" : "" }`} onClick={() => handleMatchingBand(3)}>3</span>
                      </div>
                    </div>
      ...
      {activestep === 2 && (
        <div className="popup-overlay w-full p-4" onClick={() => setactivestep(1)}>
          <div className="popup-content w-full" style={{ maxWidth: '1320px', maxHeight: 'calc(100vh - 100px)', overflow: 'hidden' }} onClick={(e) => e.stopPropagation()}>
      ...
      {activestep === 3 && (
        <div className="popup-overlay w-full" onClick={() => { if (GSsystum) { setSelectedRetailer(''); setRetailer('false'); } setactivestep(1); }}>
          <div className="popup-content w-4/5" style={{ maxWidth: '900px', maxHeight: 'calc(100vh - 100px)', overflow: 'auto' }} onClick={(e) => e.stopPropagation()}>
            <button className="close-btn" onClick={() => { if (GSsystum) { setSelectedRetailer(''); setRetailer('false'); } setactivestep(1); }}>
              <i className="fas fa-times"></i>
            </button>
            {retailer === 'true' ? (
            <div className='w-full'>
              <div className="popup-left">
                <h2 className="text-2xl font-bold text-gray-800">Enter Your Contact Details</h2>
              </div>
              <form className='text-left' onSubmit={(e) => { e.preventDefault(); handleCreateTransactionSubmit(); }}>
                ...
                <button type="submit" className="action-btn block flex items-center" disabled={isLoading}>
                  {isLoading ? (
                      <span>Loading...</span>
                  ) : (
                      "Submit"
          ...
          <div className='w-full'>
            <div className="mb-6">
              <h2 className="text-2xl font-bold text-gray-800">Select a Retailer</h2>
              <p className="text-gray-600 mt-2">Choose from our list of authorized retailers</p>
            </div>
                ...
                <tbody>
                  {retailers.map(retailer => (
                    <tr key={retailer.ID} className="border-b hover:bg-gray-50">
                      <td className="p-3">
                        <div>
                          <div className="font-medium text-left">{retailer.RetailerName}</div>
      ...
      {showRegisterPopup && (
        <div className="popup-overlay" onClick={(e) => {
          if (e.target.className === 'popup-overlay') {
            ...
            <div className="popup-content" style={{ maxWidth: '900px',overflow: 'hidden' }}>
                    ...
                    <div 
                      ...
                      onClick={() => {
                        ...
                        navigator.clipboard.writeText(iframeCode).then(() => {
                          ...
                          document.getElementById('show-message').style.color = '#9333EA';
                          setTimeout(() => {
      ...
      {showPutOnMyWebsitePopup && (
        <div className="popup-overlay" onClick={() => setShowPutOnMyWebsitePopup(false)}>
          <div className="popup-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '900px',overflow: 'hidden' }}>
            <button className="close-btn" onClick={() => setShowPutOnMyWebsitePopup(false)}>
              <i className="fas fa-times"></i>
            </button>
            <h2 className="text-2xl font-bold text-gray-800 mb-4">Put On My Website</h2>
